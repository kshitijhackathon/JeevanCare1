...
export default function PersonalizedAIAvatar({ 
  ...
  // Generate stable doctor image based on patient details (cached to prevent changes)
  const getDoctorImageUrl = () => {
    ...
    // Create a stable hash from patient details to ensure consistent avatar
    const hash = patientName.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    const doctorGender = patientGender === 'male' ? 'female' : 'male';
    
    // Female doctor images for male patients (stable selection)
    if (doctorGender === 'female') {
      const femaleImages = [
        'https://images.unsplash.com/photo-1559839734-2b71ea197ec2?w=400&h=400&fit=crop&crop=face',
        'https://images.unsplash.com/photo-1582750433449-648ed127bb54?w=400&h=400&fit=crop&crop=face',
        'https://images.unsplash.com/photo-1594824475379-36f8b8df0eec?w=400&h=400&fit=crop&crop=face'
      ];
      return femaleImages[hash % femaleImages.length];
    }
    // Male doctor images for female patients (stable selection)
    else {
      const maleImages = [
