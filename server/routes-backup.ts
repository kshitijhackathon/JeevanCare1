...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Enhanced TTS Audio Generation Endpoint - DISABLED
  app.post("/api/tts/generate-disabled", async (req, res) => {
    try {
  ...
  // Fixed Medical Chat with Working Disease Detection - DISABLED
  app.post("/api/ai-doctor/groq-medical-chat-disabled", async (req, res) => {
    try {
      const { message, language = 'english' } = req.body;
      ...
      // STEP 1: Extract symptoms using basic logic
      const symptoms = message.toLowerCase().split(' ').filter(word => 
        ['fever', 'बुखार', 'headache', 'सिरदर्द', 'cough', 'खांसी', 'pain', 'दर्द'].includes(word)
      );
      ...
      // STEP 2: Basic disease prediction
      const prediction = { disease: 'Common Cold', confidence: 70, severity: 'mild' };
